...
c u =	Hubbard repulsion
	open(22,file="input.bin", form= "formatted", status="old")

 	read(22,*)f,d0,u
c************************************************
	open(25,file="rho", form= "formatted", status="unknown")
	open(23,file="Sigma", form= "formatted", status="unknown")


c output files
c fort.25 is Im[G(w)]   (the DOS)
c fort.23 is Re[Sigma(w)],Im[Sigma(w)]
...
c FFT from omega to time
        do i=1,2*L
        data(2*i-1)=dreal(g0(i))
        data(2*i)=dimag(g0(i))
        enddo
        call four1(data,L2,1)
        do i=1,2*L
        tg0(i)=dcmplx(data(2*i-1),data(2*i))
        enddo
...
c FFT from time to omega 
        ...
        do i=1,2*L
        sf2(i)=sft2(i)
        data(2*i-1)=dreal(sft2(i))
        data(2*i)=dimag(sft2(i))
        enddo
        call four1(data,L2,-1)
        do i=1,2*L
        sf2(i)=dcmplx(data(2*i-1),data(2*i))
        enddo
...
c fort.26 is Im[G(w)]  (impurity problem, no self-consistency, imet=1)
	...
	do  i=imax2,imax1+1,-i02
        ...
        write(23,*)-(f*float(i)),(dreal(sf(-i))),(dimag(sf(-i)))
	write(27,*)-(f*float(i)),(dreal(g0(i+L+1)))
	write(28,*)-(f*float(i)),(dimag(g0(i+L+1)))
	enddo
...
100	continue	
	...
	print*," 'rho' is Im[G(w)]   (the DOS)"
        print*," 'Sigma' is Re[Sigma(w)],Im[Sigma(w)] "
